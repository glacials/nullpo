...
<?php if ($this->_rootref['PROMPT']) {  ?>
	...
	<table class="tablebg" width="100%" cellspacing="<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_SPACING'])) ? $this->_tpldata['DEFINE']['.']['CA_SPACING'] : ''; ?>" cellpadding="0" border="0">
	<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_CAP_START'])) ? $this->_tpldata['DEFINE']['.']['CA_CAP_START'] : ''; echo ((isset($this->_rootref['L_OPTIONS'])) ? $this->_rootref['L_OPTIONS'] : ((isset($user->lang['OPTIONS'])) ? $user->lang['OPTIONS'] : '{ OPTIONS }')); echo (isset($this->_tpldata['DEFINE']['.']['CA_CAP_END'])) ? $this->_tpldata['DEFINE']['.']['CA_CAP_END'] : ''; ?>
...
<?php } else { if ($this->_rootref['S_PM_ICONS']) {  $_messagerow_count = (isset($this->_tpldata['messagerow'])) ? sizeof($this->_tpldata['messagerow']) : 0;if ($_messagerow_count) {for ($_messagerow_i = 0; $_messagerow_i < $_messagerow_count; ++$_messagerow_i){$_messagerow_val = &$this->_tpldata['messagerow'][$_messagerow_i]; if ($_messagerow_val['PM_ICON_IMG']) {  $this->_tpldata['DEFINE']['.']['CA_HAS_ICONS'] = 1; } }} if (! $this->_tpldata['DEFINE']['.']['CA_HAS_ICONS']) {  $this->_tpldata['DEFINE']['.']['CA_EXTRA_COL'] = ' colspan="2"'; } } ?>

	<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_BLOCK_START'])) ? $this->_tpldata['DEFINE']['.']['CA_BLOCK_START'] : ''; ?>

	<table class="tablebg" width="100%" cellspacing="<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_SPACING'])) ? $this->_tpldata['DEFINE']['.']['CA_SPACING'] : ''; ?>" cellpadding="0" border="0">
	<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_CAP_START'])) ? $this->_tpldata['DEFINE']['.']['CA_CAP_START'] : ''; if ($this->_rootref['CUR_FOLDER_NAME']) {  echo (isset($this->_rootref['CUR_FOLDER_NAME'])) ? $this->_rootref['CUR_FOLDER_NAME'] : ''; } else { echo ((isset($this->_rootref['L_PRIVATE_MESSAGES'])) ? $this->_rootref['L_PRIVATE_MESSAGES'] : ((isset($user->lang['PRIVATE_MESSAGES'])) ? $user->lang['PRIVATE_MESSAGES'] : '{ PRIVATE_MESSAGES }')); } echo (isset($this->_tpldata['DEFINE']['.']['CA_CAP_END'])) ? $this->_tpldata['DEFINE']['.']['CA_CAP_END'] : ''; ?>
	...
	<tr>
		...
		<th style="border-left-width: 0;" colspan="<?php if ($this->_rootref['S_PM_ICONS']) {  ?>2<?php } else { ?>1<?php } ?>">&nbsp;<?php echo ((isset($this->_rootref['L_SUBJECT'])) ? $this->_rootref['L_SUBJECT'] : ((isset($user->lang['SUBJECT'])) ? $user->lang['SUBJECT'] : '{ SUBJECT }')); ?>&nbsp;</th>
	...
	<?php $_messagerow_count = (isset($this->_tpldata['messagerow'])) ? sizeof($this->_tpldata['messagerow']) : 0;if ($_messagerow_count) {for ($_messagerow_i = 0; $_messagerow_i < $_messagerow_count; ++$_messagerow_i){$_messagerow_val = &$this->_tpldata['messagerow'][$_messagerow_i]; ?>
		...
		<tr>
			<td class="row1<?php if (! $_messagerow_val['PM_IMG'] && $_messagerow_val['PM_CLASS']) {  ?> <?php echo $_messagerow_val['PM_CLASS']; ?>_left<?php } ?>" width="25" align="center" nowrap="nowrap"><?php echo $_messagerow_val['FOLDER_IMG']; ?></td>
		<?php if ($this->_tpldata['DEFINE']['.']['CA_HAS_ICONS']) {  ?>

			<td class="row1" width="25" align="center"><?php if ($_messagerow_val['PM_ICON_IMG']) {  echo $_messagerow_val['PM_ICON_IMG']; } else { ?>&nbsp;<?php } ?></td>
		<?php } if ($_messagerow_val['S_PM_DELETED']) {  ?><td class="row3"<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_EXTRA_COL'])) ? $this->_tpldata['DEFINE']['.']['CA_EXTRA_COL'] : ''; ?>><?php } else { ?><td class="row1"<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_EXTRA_COL'])) ? $this->_tpldata['DEFINE']['.']['CA_EXTRA_COL'] : ''; ?>><?php } if ($_messagerow_val['PM_IMG']) {  ?>
			...
			<td class="row1<?php if (! $_messagerow_val['PM_IMG'] && $_messagerow_val['PM_CLASS']) {  ?> <?php echo $_messagerow_val['PM_CLASS']; ?>_right<?php } ?>" width="20" align="center"><p class="topicdetails"><input type="checkbox" class="radio" name="marked_msg_id[]" value="<?php echo $_messagerow_val['MESSAGE_ID']; ?>" /></p></td>
		</tr>
	<?php }} else { ?>
...
<tr>
	<td class="cat" colspan="<?php echo (isset($this->_tpldata['DEFINE']['.']['COLSPAN'])) ? $this->_tpldata['DEFINE']['.']['COLSPAN'] : ''; ?>">
