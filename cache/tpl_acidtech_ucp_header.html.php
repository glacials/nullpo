<?php if (!defined('IN_PHPBB')) exit; $this->_tpldata['DEFINE']['.']['CA_SKIP_PM_NOTIFY'] = '1'; $this->_tpl_include('overall_header.html'); if ($this->_rootref['S_SHOW_PM_BOX'] && $this->_rootref['S_POST_ACTION']) {  ?>
...
<?php if ($this->_rootref['S_SHOW_PM_BOX'] && $this->_rootref['S_POST_ACTION']) {  ?>
	...
	<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_CAP_START'])) ? $this->_tpldata['DEFINE']['.']['CA_CAP_START'] : ''; echo ((isset($this->_rootref['L_PM_TO'])) ? $this->_rootref['L_PM_TO'] : ((isset($user->lang['PM_TO'])) ? $user->lang['PM_TO'] : '{ PM_TO }')); echo (isset($this->_tpldata['DEFINE']['.']['CA_CAP_END'])) ? $this->_tpldata['DEFINE']['.']['CA_CAP_END'] : ''; ?>
		...
		<?php if ($_l_block1_val['S_SELECTED']) {  ?>
			...
			<?php if ($this->_rootref['S_PRIVMSGS']) {  ?><!-- the ! at the beginning of the loop name forces the loop to be not a nested one of l_block1 (it gets parsed separately) --><?php $_folder_count = (isset($this->_tpldata['folder'])) ? sizeof($this->_tpldata['folder']) : 0;if ($_folder_count) {for ($_folder_i = 0; $_folder_i < $_folder_count; ++$_folder_i){$_folder_val = &$this->_tpldata['folder'][$_folder_i]; if ($_folder_val['S_FIRST_ROW']) {  ?>
					...
					<?php } if ($_folder_val['S_CUR_FOLDER']) {  ?>
						...
						<li class="folder-current" style="padding: 1px 0;">&#187; <a href="<?php echo $_folder_val['U_FOLDER']; ?>"><?php echo $_folder_val['FOLDER_NAME']; if ($_folder_val['S_UNREAD_MESSAGES']) {  ?> (<?php echo $_folder_val['UNREAD_MESSAGES']; ?>)<?php } ?></a></li>
					<?php } else { ?>
...
<?php if ($this->_rootref['S_SHOW_COLOUR_LEGEND']) {  ?>
	...
	<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_CAP_START'])) ? $this->_tpldata['DEFINE']['.']['CA_CAP_START'] : ''; echo ((isset($this->_rootref['L_MESSAGE_COLOURS'])) ? $this->_rootref['L_MESSAGE_COLOURS'] : ((isset($user->lang['MESSAGE_COLOURS'])) ? $user->lang['MESSAGE_COLOURS'] : '{ MESSAGE_COLOURS }')); echo (isset($this->_tpldata['DEFINE']['.']['CA_CAP_END'])) ? $this->_tpldata['DEFINE']['.']['CA_CAP_END'] : ''; ?>
...
<?php } if ($this->_rootref['S_ZEBRA_ENABLED'] && $this->_rootref['S_ZEBRA_FRIENDS_ENABLED']) {  ?>
	...
	<table class="tablebg" width="100%" cellspacing="<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_SPACING'])) ? $this->_tpldata['DEFINE']['.']['CA_SPACING'] : ''; ?>">
	<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_CAP_START'])) ? $this->_tpldata['DEFINE']['.']['CA_CAP_START'] : ''; echo ((isset($this->_rootref['L_FRIENDS'])) ? $this->_rootref['L_FRIENDS'] : ((isset($user->lang['FRIENDS'])) ? $user->lang['FRIENDS'] : '{ FRIENDS }')); echo (isset($this->_tpldata['DEFINE']['.']['CA_CAP_END'])) ? $this->_tpldata['DEFINE']['.']['CA_CAP_END'] : ''; ?>
	...
	<tr>
		<td class="row1" align="center">
			...
			<b class="genmed error"><?php echo ((isset($this->_rootref['L_FRIENDS_OFFLINE'])) ? $this->_rootref['L_FRIENDS_OFFLINE'] : ((isset($user->lang['FRIENDS_OFFLINE'])) ? $user->lang['FRIENDS_OFFLINE'] : '{ FRIENDS_OFFLINE }')); ?></b>
