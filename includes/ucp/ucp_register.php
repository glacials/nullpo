...
class ucp_register
{
	...
	function main($id, $mode)
	{
		...
		if (!empty($register_key) && $invite->valid_key($register_key) && $invite->config['set_cookie'])
		{
			$invite->set_cookie($register_key);
		}
		$cookie_key		= (isset($_COOKIE[$config['cookie_name'] . '_reg_key'])) ? $_COOKIE[$config['cookie_name'] . '_reg_key'] : '';
		$register_key 	= (!empty($cookie_key)) ? $cookie_key : $register_key;
		...
		if (!$agreed || ($coppa === false && $config['coppa_enable']) || ($coppa && !$config['coppa_enable']))
		{
			...
			$template->assign_vars(array('REGISTER_KEY'		=> $register_key));
			unset($lang_row);
		...
		if ($submit)
		{
			...
			if (!sizeof($error))
			{
				...
				if (!empty($register_key) && $invite->valid_key($register_key, false))
				{
				...
				else if (!empty($register_key) && $invite->valid_key($register_key, false) && $invite->config['invite_require_activation'] == USER_ACTIVATION_NONE)
				{
				...
				if ($config['email_enable'])
				{
					...
					if (($config['require_activation'] == USER_ACTIVATION_ADMIN) ||
						(!empty($register_key) && $invite->valid_key($register_key, false) && $invite->config['invite_require_activation'] == USER_ACTIVATION_ADMIN))
					{
		...
		if ($invite->config['display_registration'])
		{
		...
		$template->assign_vars(array(
			...
			'S_UCP_ACTION'		=> append_sid("{$phpbb_root_path}ucp.$phpEx", $url_key . 'mode=register'),

			'REGISTER_KEY'				=> $register_key,
			'S_DISPLAY_REGISTER_KEY'	=> (!empty($register_key) && $invite->valid_key($register_key) && $invite->config['autohide_valid_key']) ? false : ((!$invite->config['enable']) ? false : (($invite->config['display_registration']) ? true : false)),
			'S_ENABLE_KEY_OPTIONAL'		=> (!$invite->config['enable']) ? false : (($invite->config['enable_key'] == OPTIONAL) ? true : false),
		));
