...
function get_username_string($mode, $user_id, $username, $username_colour = '', $guest_username = false, $custom_profile_url = false)
{
	...
	// SKOH-FLEY NAME SUFFIXES START
		if(file_exists("config.php")) { // only perform if we're not in the admin panel
		
				include("config.php");
				mysql_connect($dbhost, $dbuser, $dbpasswd);
				mysql_select_db($dbname);
				
				$dbhost = ""; // security clean-ups
				$dbuser = ""; // just in case
				$dbpasswd = "";
				$dbname = "";
				...
				if(!function_exists("user_in_group")) {
						function user_in_group($user_id, $group_id) {
								$query = mysql_query('SELECT user_id FROM ' . USER_GROUP_TABLE . ' WHERE user_id = ' . $user_id . ' AND group_id = ' . $group_id);
				...
				$suffixes = '';
				if(user_in_group($user_id, 867)) // founder
						$suffixes .= '^';
				if(user_in_group($user_id, 877)) // moderator
						$suffixes .= '`';
				if(user_in_group($user_id, 880)) // uber veteran
						$suffixes .= '-';
				if(user_in_group($user_id, 873)) // uber
						$suffixes .= '*';

				//$suffixes = '<span style="color: #aaaaaa;">' . $suffixes . '</span>';
		}
	// SKOH-FLEY NAME SUFFIXES END
	...
	switch ($mode)
	{
		...
		case 'username':
			...
			if ($mode == 'username')
			{
				return $username . $suffixes; // skoh-fley name suffixes
			}
	...
	{ // skoh-fley suffixes in the next two return statements (search for $suffixes)
		return str_replace(array('{USERNAME_COLOUR}', '{USERNAME}'), array($username_colour, $username . $suffixes), (!$username_colour) ? $_profile_cache['tpl_noprofile'] : $_profile_cache['tpl_noprofile_colour']);
	}

	return str_replace(array('{PROFILE_URL}', '{USERNAME_COLOUR}', '{USERNAME}'), array($profile_url, $username_colour, $username . $suffixes), (!$username_colour) ? $_profile_cache['tpl_profile'] : $_profile_cache['tpl_profile_colour']);
}
